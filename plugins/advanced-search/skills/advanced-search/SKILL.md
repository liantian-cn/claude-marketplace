...
All four engines are attempted on every search. If any engine is unavailable (API key not configured, auth error, service down, tool not found), the search continues with the remaining engines. This rule applies to ALL engines equally.

**Degradation rules:**
- **4 engines available** → Full coverage: breadth + depth + Chinese + semantic
- **3 engines available** → Reduced but still comprehensive — missing one dimension (e.g., no Bocha = no semantic freshness-filtered results)
...
- **0 engines available** → **严格告知用户：** "无任何可靠信息来源，无法完成搜索。请检查网络连接、API Key 配置（TAVILY_API_KEY、DASHSCOPE_API_KEY、BOCHA_API_KEY）以及 MCP 服务状态。"
...
**Note on Bocha pricing:** Bocha is a paid/premium search service. If the user hasn't configured `BOCHA_API_KEY`, the Bocha subagent will fail — this is expected. The other three engines (WebSearch, Tavily, Bailian) will still provide comprehensive coverage.

## Core principle

**Every web information need goes through this skill.** Don't reach for WebSearch, individual tavily-* tools, bailian_web_search, or bocha_web_search — this skill orchestrates them better than you can manually, because it:
- Runs all four engines simultaneously (no serial waiting)
...
**When results arrive:** Show WebSearch, Bailian, and Bocha results immediately. The Tavily research report takes 30-120s — note "Deep research report being prepared..." while waiting. Merge all results when the report arrives. Bocha results often surface unique sources due to its semantic matching.
...
When results from all four engines come back:
...
- Mark source provenance for each result: `[Tavily]`, `[WebSearch]`, `[Bailian]`, `[Bocha]`, or combined tags like `[Tavily+Bocha]`, `[All 4]`
...
**Sources:** X unique results (A from all four, B from Tavily only, C from WebSearch only, D from Bailian only, E from Bocha only, ...)
...
- Bailian-only and Bocha-only results are particularly valuable — call them out
...
**Do NOT** run engines sequentially. All four engines have complementary strengths — run them together. The subagent pattern means even slow Tavily operations (research, crawl) don't block the main conversation. Bailian, Bocha, and WebSearch typically return first, giving immediate results while Tavily deep research continues.
...
- **For Chinese-language queries**, Bailian and Bocha fill critical gaps — the other two engines have weaker Chinese coverage. Search with Chinese keywords even if the user asked in English.
- **For time-sensitive queries**, Bocha's `freshness` filter and publication dates are unique advantages — use them. Bailian has no time filter, so compensate with query crafting.
...
- **Tavily research takes 30-120 seconds** — show WebSearch, Bailian, and Bocha results immediately and note that the deep research report is being prepared.
...
- **Engine failures are normal** — Bocha requires a paid API key, and any engine can be temporarily unavailable. Always note which engines contributed and which were unavailable. Never fail the entire search because one engine is down.
